...
const { adminModel, electionModel, questionModel, optionModel, voterModel } = require("./models");
  ...
  async (request, response) => {
    if (request.user.role === "admin") {
      try {
        ...
        const numberOfQuestions = await questionModel.getNumberOfQuestions(
          request.params.id
        );
...
app.get(
  ...
  async (request, response) => {
    if (request.user.role === "admin") {
      try {
        const election = await electionModel.getElection(request.params.id);
        const questions = await quesitonModel.getQuestions(request.params.id);
        if (!election.running) {
          if (request.accepts("html")) {
  ...
  async (request, response) => {
    if (request.user.role === "admin") {
      ...
      try {
        ...
        const question = await quesitonModel.addQuestion({
  ...
  async (request, response) => {
    if (request.user.role === "admin") {
      try {
        ...
        const question = await quesitonModel.getQuestion(request.params.questionID);
...
app.delete(
  ...
  async (request, response) => {
    if (request.user.role === "admin") {
      try {
        const nq = await questionModel.getNumberOfQuestions(
          request.params.electionID
        );
        if (nq > 1) {
          const res = await questionModel.deleteQuestion(request.params.questionID);
...
app.get(
  ...
  async (request, response) => {
    if (request.user.role === "admin") {
      try {
        const question = await questionModel.getQuestion(request.params.questionID);
...
app.get("/e/:url/", async (request, response) => {
  ...
  try {
    const election = await electionModel.getElectionURL(request.params.urlString);
    if (request.user.role === "voter") {
      if (election.launch) {
        const questions = await questionModel.getQuestions(election.id);
        let options = [];
        for (let question in questions) {
